...
public class AssetBundlesEditorTools : MonoBehaviour
{
    ...
    static void GenerateResZipImp(string rootDir)
    {
        ...
        //----------------------------------------------------//
        //写到发布目录
        string xmlContent = VersionInfo2.SerializeInEditor(listResInfo);
        string xmlFilePath;
        if (PackageWizard.m_wizard)
        {
        ...
        xmlContent = VersionInfo2.SerializeInEditor(listResInfo);
        //写到工程目录
        string xmlFileInProjectPath = rootDir + "Assets/StreamingAssets/Config/" + VersionManager2.VersionInfoFilePath + MyFileUtil.EncryptXMLFileSuffix;
