...
public class NGUIJson
{
	...
	/// </summary>
    /*
    public static List<UIAtlas.SpriteExt> LoadSpriteData(string jsonString)
	{
        if (jsonString == null)
        {
            return null;
		...
		if (decodedHash == null)
		{
			Debug.LogWarning("Unable to parse Json file: " + jsonString);
			return null;
		}

        List<UIAtlas.SpriteExt> spriteList = new List<UIAtlas.SpriteExt>();
        ...
        {
            Hashtable tableSize = (Hashtable)meta["size"];
            atlasHeight = int.Parse(tableSize["h"].ToString());
        }

		Hashtable frames = (Hashtable)decodedHash["frames"];
		foreach (System.Collections.DictionaryEntry item in frames)
		{
            UIAtlas.SpriteExt newSprite = new UIAtlas.SpriteExt();
			newSprite.spriteName = item.Key.ToString();
            newSprite.spriteName = newSprite.spriteName.Replace(".png", "");
            newSprite.spriteName = newSprite.spriteName.Replace(".tga", "");
            ...
            frameY = atlasHeight - frameY - frameH; //转换坐标，从左上角转为左下角
            newSprite.textureRect = new Rect(frameX, frameY, frameW, frameH);
...
// 			}
    ...
    */
	...
	static int CompareSprites (Sprite a, Sprite b) { return a.name.CompareTo(b.name); }
